const state = { directory: localStorage.getItem('echonote.directory') || '', recorder: null, chunks: [], startedAt: null, timerId: null, lastFiles: null };
...
async function startRecording() {
  if (!state.directory) { toast('请先设置保存目录'); await chooseDirectory(); if (!state.directory) return; }
  try {
    ...
    state.recorder.start(); $('record-card').classList.add('recording'); $('recordButton').classList.add('recording'); $('recordButtonIcon').textContent = '■'; $('recordButtonText').textContent = '录音中'; $('stopButton').disabled = false; $('recordTitle').textContent = '正在记录系统声音'; $('recordHint').textContent = '结束后会自动进入转录流程。'; setStatus('RECORDING', true); state.timerId = window.setInterval(() => setTimer(Math.floor((Date.now() - state.startedAt) / 1000)), 500);
  } catch (error) { toast(error.message || '无法开始录音'); setIdle(); }
}
function stopRecording() { if (state.recorder?.state === 'recording') { window.clearInterval(state.timerId); state.recorder.stop(); } }
...
setDirectory(state.directory); $('recordButton').addEventListener('click', startRecording); $('stopButton').addEventListener('click', stopRecording); $('setupButton').addEventListener('click', chooseDirectory); $('folderButton').addEventListener('click', chooseDirectory); $('deleteButton').addEventListener('click', deleteLast); $('settingsButton').addEventListener('click', chooseDirectory);
